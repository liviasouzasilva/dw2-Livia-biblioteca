...
// Spinner util
function withSpinner(buttonEl, asyncFn){
    const orig = buttonEl.innerHTML;
    buttonEl.disabled = true;
    buttonEl.innerHTML = '<span class="spinner" aria-hidden="true"></span>';
    return asyncFn().finally(()=>{ buttonEl.disabled=false; buttonEl.innerHTML = orig; });
}

// Hook do modal para trap focus
const modal = document.getElementById('modal-livro');
if(modal){
    modal.addEventListener('close', ()=>{
        // devolver foco ao botão Novo Livro
        document.getElementById('novo-livro').focus();
    });
    modal.addEventListener('open', ()=> trapFocus(modal));
}

// Atualizar handler de submit para usar spinner e toasts
const formLivro = document.getElementById('form-livro');
if(formLivro){
    formLivro.addEventListener('submit', function(e){
        ...
        withSpinner(btn, async ()=>{
            // reutiliza handleFormSubmit para lógica de envio
            try{
...
// atalhos globais (Alt+N abrir modal)
document.addEventListener('keydown', (e)=>{
